// NOTE: THIS FILE IS NOT USED IN ANY WAY BY THE LIBRARY
//
//
// THE CODE IN THIS FILE HAS BEEN LEFT HERE TO SERVE AS A RECALL AID
// TO THE ORIGINAL OPTIMIZATION PROBLEM THAT SPURRED THE CREATION OF THIS LIBRARY
...
users_with_no_team_factor = 0.0     // Add this percentage of users that have no team
                                    // (use values from 0 to INF) example: 0.25 (adds 0.25 more users).
...
// Return an array of the indexes of the elements
function indexes_for_elements(all_elements, elements){
  return _.map(highlighted_elements, function(element){
...
function print(all_elements, bitmap, highlighted_elements, chunk_size = 10){

  highlighted_element_indexes = indexes_for_elements(all_elements, highlighted_elements)
  ...
  bits = bitmap_to_string(all_elements, bitmap).split('') // Split to bits
...
function replaceCharAt(string, index, char, implementation = 'substring'){
  if(implementation === 'substring'){
    if(index > string.length-1) return string;
    return string.substr(0,index) + char + string.substr(index+1);
  }else if (implementation = 'array_join'){
    arrayJoinReplaceCharAt()
  }
}

function substringReplaceCharAt(string, index, char){
  if(index > string.length-1) return string;
...
function arrayJoinReplaceCharAt(string, index, char){
  if(index > string.length-1) return string;
  letters = string.split('')
  letters[index] = char
  return letters.join('')
...
function test(){
  ...
  assert.equal("X234567890", arrayJoinReplaceCharAt("1234567890", 0, "X"), "arrayJoinReplaceCharAt - Test 1 failed")
  assert.equal("1234X67890", arrayJoinReplaceCharAt("1234567890", 4, "X"), "arrayJoinReplaceCharAt - Test 2 failed")
  assert.equal("123456789X", arrayJoinReplaceCharAt("1234567890", 9, "X"), "arrayJoinReplaceCharAt - Test 3 failed")
}
