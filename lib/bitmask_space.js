...
class BitmaskSpace {
  constructor(elements, options = {}) {
    ...
    if(!Array.isArray(elements)){ throw Error('The space defined by all elements must be an array!') }
    // Check if elements is actually a value
    if(elements.length === 0){ throw Error("The space defined by all elements cannot be empty!") }
  ...
  // The space is composed of all 1's by definition
  toString(){
    return this.ones;
  }
