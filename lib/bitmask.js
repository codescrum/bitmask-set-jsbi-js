...
class Bitmask {
  ...
  constructor (set, elements, options = { inherit_sort: true, sort: false }) {
    ...
    // Associate with set
    this.set = set // associates it to a set
    this.size = this.set.size // size must be the same as the set size
    ...
    if (Array.isArray(elements) && !(elements instanceof JSBI)) {
      ...
      // repect the order of elements as it is on the set
  ...
  add (elements) {
    return this.set.bitmask(JSBI.bitwiseOr(this.bits, this.set.bitmask(elements).bits))
  }

  remove (elements) {
    return this.set.bitmask(JSBI.bitwiseAnd(this.bits, this.set.bitmask(elements).invert().bits))
  }
  ...
  compute_elements () {
    ...
    string_representation.split('').forEach((bit, index) => {
      if (bit === '1') {
        elements.push(this.set.elements[index])
      }
    })
  ...
  elements () {
    this._elements || this.compute_elements()
  }
