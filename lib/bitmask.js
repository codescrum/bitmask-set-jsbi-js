...
class Bitmask {
  ...
  constructor(space, elements, options = {}) {
    ...
    if(space == undefined || space == null){ throw Error("You need to specify a space for the bitmask to make sense.") }

    /////// Initialize

    // Associate with space
  ...
  indexOf(element){
    return this.space.indexOf(element)
  }
  ...
  //            which belong to the same space!
  //
  ...
  and(other){
    return this.space.bitmask(JSBI.bitwiseAnd(this.bits, other.bits));
  }

  or(other){
    return this.space.bitmask(JSBI.bitwiseOr(this.bits, other.bits));
  }

  xor(other){
    return this.space.bitmask(JSBI.bitwiseXor(this.bits, other.bits));
  }

  not(){
    throw('Not supported - elements must reside in the space (check Galois Field)');
    return this.space.bitmask(JSBI.bitwiseNot(this.bits));
  }

  invert(){
    return this.space.bitmask(JSBI.BigInt('0b' + this.flip_string_bits(this.toString())))
  }
  ...
  add(elements){
    return this.space.bitmask(JSBI.bitwiseOr(this.bits, this.space.bitmask(elements).bits))
  }

  remove(elements){
    return this.space.bitmask(JSBI.bitwiseOr(this.bits, this.space.bitmask(elements).invert().bits))
  }
  ...
  compute_elements(){
    ...
    _.each(string_representation.split(''), (bit, index) => {
      if(bit === '1'){
        elements.push(this.space.elements[index])
      }
    });
  ...
  elements_to_bitmask_string(elements) {
    let bitmask_array = this.space.zeros.split('') // Create array of 0's
