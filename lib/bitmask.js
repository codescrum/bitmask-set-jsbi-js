...
class Bitmask {
  ...
  constructor(field, elements, options = {}) {
    ...
    if(Array.isArray(elements) && !(elements instanceof JSBI)){
      ...
      let string =  elements; // Let's call it string for readability.
      ...
      this.bigint_representation = JSBI.BigInt("0b" + this.string_representation)
      this.bits = this.bigint_representation;
  ...
  not_in(other){
     // When there are no elements, it is always contained in another bitmask.
    if(JSBI.equal(this.bits, JSBI.BigInt(0))){
      return false;
    }else{
