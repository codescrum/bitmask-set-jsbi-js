...
class Bitmask {
  ...
  constructor(space, elements, options = {}) {
    ...
    }else if(typeof(elements) == 'bigint'){
  ...
  and(other){
    return this.space.bitmask(this.bits & other.bits);
  }
  ...
  not(){
    throw('Not supported');
    return this.space.bitmask(~this.bits);
  }

  invert(){
    return this.space.bitmask(BigInt('0b' + this.flip_string_bits(this.toString())))
  }
  ...
  add(elements){
    return this.space.bitmask(this.bits | this.space.bitmask(elements).bits)
  }

  remove(elements){
    return this.space.bitmask(this.bits | this.space.bitmask(elements).invert().bits)
  }
